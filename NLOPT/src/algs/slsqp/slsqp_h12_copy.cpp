...
/*            IF NCV <= 0 NO OPERATIONS WILL BE DONE ON C(). */
    ...
    //cl = (d__1 = u[*lpivot * u_dim1 + 1], fabs(d__1));
    //cl = (d__1 = temp_matrix[*lpivot - 1][*lpivot - 1], fabs(d__1));
...
/*     ****** CONSTRUCT THE TRANSFORMATION ****** */
	...
	// Modified 1
	//double temp_val = temp_matrix[*lpivot - 1][j - 1];
	//sm = (d__1 = temp_val, fabs(d__1));
	//sm1 += temp_val * temp_val;
...
/* Computing 2nd power */
	...
	//double temp_val = temp_matrix[*l1 - 2][j - 1];
	//d__1 = temp_val * clinv;

	d__1 = temp_matrix[*l1 - 2][j - 1] * clinv;
	sm += d__1 * d__1;
    }
...
/*  TRIANGULARIZE C AND APPLY FACTORS TO E AND G */
    ...
    double **temp_matrix = (double **)malloc(c_dim1 * sizeof(double *));
...
/* Computing MIN */
	...
	h12_copy(&c__2, &i__, &i__2, n, &c__[i__ + c_dim1], lc, &w[iw + i__], &e[
		e_offset], le, &c__1, me, temp_matrix);
	//h12t2 = clock();
...
/* L10: */
	...
	h12_copy(&c__2, &i__, &i__2, n, &c__[i__ + c_dim1], lc, &w[iw + i__], &g[
		g_offset], lg, &c__1, mg, temp_matrix);
	//h12t2 = clock();
