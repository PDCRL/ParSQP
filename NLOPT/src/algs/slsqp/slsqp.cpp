...
//#include "/home/rahul/temp_rahul/ParSQP/Parallel_QR/main.cpp"
#include "slsqp.h"
#include <iostream>
#include <fstream>
#include <vector>
...
L40:
    ...
    #pragma omp parallel for num_threads(5) private(i__, sm)
    for (j = 1; j <= i__1; ++j) {
...
static void standalone_householder(int n, int m, double *e, std::vector<double> &up_array) 
{
    const double one = 1.;
    int i__, i__2, j, i__3;
    int lpivot, l1;
...
/*  QR-FACTORS OF E AND APPLICATION TO F */
    ...
    for (i__ = 1; i__ <= i__1; ++i__) {
        ...
        i__2 = i__ + 1;
        i__3 = *n - i__;
        h12_(&c__2, &i__, &i__2, me, &e[i__ * e_dim1 + 1], &c__1, &up_array[i__ - 1], &f[1],
            &c__1, &c__1, &c__1);
    }
...
/*  TRANSFORM G AND H TO GET LEAST DISTANCE PROBLEM */
    *mode = 5;
    ...
    // std::stringstream savefile1, savefile2;
    // savefile1<<"gdata/g_temp_matrix_before_"<<gctr<<".csv";
    // std::cout << "lg " << *lg << ", n " << *n << std::endl;
/*
...
/*
    savefile2<<"gdata/g_temp_matrix_after_"<<gctr<<".csv";
    write_to_csv(g_temp_matrix, *lg, lsei_n, savefile2.str());
