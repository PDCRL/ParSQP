...
double merge_reduce(int start_one, int end_one, int start_two, int end_two, std::vector<int> &col_idx, std::vector<double> &data, int *lpivot, double *up)
{
    //printf("Inside Merge Reduce\n");
    double sm = 0.0;
	...
	sm = 0;
        ...
        sm = data[start_two-1] * *up;
    
    int ctr1 = start_one;
    int ctr2 = start_two;
    
    while (ctr1 < end_one && ctr2 < end_two)
    {
        if ( col_idx[ctr1] < col_idx[ctr2])
        {
            ctr1++;
        }
        else if (col_idx[ctr1] > col_idx[ctr2])
        {
            ctr2++;
        }
        else
        {
            sm += data[ctr1] * data[ctr2];
            ctr1++;
            ctr2++;
        }
    }
...
static void h12_copy(const int *mode, int *lpivot, int *l1, 
		 int *m, double *u, const int *iue, double *up, 
		 double *c__, const int *ice, const int *icv, const int *ncv, std::vector<int> &pivot_ptr, std::vector<int> &row_ptr, std::vector<int> &col_idx, std::vector<double> &data)
{
    /* Initialized data */
    /*
    // The temp_matrix which we copied is equal to the original constraint matrix
    for(int i = 0; i < *iue; i++) {
	for(int j = 0; j < *m; j++) {
		if(temp_matrix[i][j] != u[(j * *iue) + i]) {
			printf("%d, %d -- %d\n", i, j, (j * *iue) + i);
		}
	}
    }
...
L40:
    ...
    if (*mode == 1)
    {
	//std::cout << "Inside mode 1\n";
        for (j = *lpivot; j < row_ptr.size() - 1; j++)
        {

	    if(j == 338){
		//std::cout << "here\n";
	    }
            int c_start_two = row_ptr[j];
            int c_end_two = row_ptr[j + 1];
            
            sm = merge_reduce(c_start+1, c_end, c_start_two, c_end_two, col_idx, data, lpivot, up);
            
            if (sm == 0.0)
                continue;
            //std::cout << "pivot : " << *lpivot << " , j : " << j << "\n";
            sm *= b;
            merge_update(c_start+1, c_end, c_start_two, c_end_two, row_ptr, col_idx, data, sm, j+1, lpivot, up);
	}
        
    }
    else
    {
	//std::cout << "Inside mode 2\n";
        for (j = 1; j <= i__1; ++j) {
	    i2 += *icv;
	    i3 = i2 + incr;
	    i4 = i3;
	    ...
	    i__2 = *m;
            ...
            for (i__ = c_start+1; i__ < c_end; i__++){
                sm += c__[i3 + ((col_idx[i__] - *lpivot)* *ice)] * data[i__];
            }
            ...
            c__[i2] += sm * *up;
            for (i__ = c_start+1; i__ < c_end; i__++){
                c__[i3 + ((col_idx[i__] - *lpivot)* *ice) ] += sm * data[i__];
	    }
	...
	if(*mode == 1) {
		...
		for (i__ = *l1; i__ <= i__2; ++i__) {
		    //sm += c__[i3] * u[i__ * u_dim1 + 1];
		    sm += c__[i3+(i__- 1 - *lpivot)* *ice] * temp_matrix[*lpivot - 1][i__ - 1];
		    //i3 += *ice;
		}
		if (sm == 0.0) {
		    goto L70;
		...
		for (i__ = *l1; i__ <= i__2; ++i__) {
		    //c__[i4] += sm * u[i__ * u_dim1 + 1];
		    c__[i4 + (i__ - 1 - *lpivot)* *ice] += sm * temp_matrix[*lpivot - 1][i__ - 1];
		    //i4 += *ice;
		}	
...
/* STEP FOUR (TEST INDX J FOR LINEAR DEPENDENCY) */
    ...
    i__2 = npp1 + 1;
	//printf("1st h12 instance\n");
	h12t1 = clock();
    h12_(&c__1, &npp1, &i__2, m, &a[j * a_dim1 + 1], &c__1, &up, &z__[1], &
	    c__1, &c__1, &c__0);
	h12t2 = clock();
	//printf("time taken by h12 function : %0.5f\n", (double)(h12t2 - h12t1) / CLOCKS_PER_SEC);
...
/*  TRIANGULARIZE C AND APPLY FACTORS TO E AND G */
    i__1 = *mc;
	////printf("Inside slsqp: if mode == -1: 17\n");
	//printf("total number of iterations for 10, 11, 12 instance execution - %d\n", i__1);
	//h12t1 = clock();

	printf("I am here 1: 1320\n");

   /* 
    double **sparse_matrix = (double **)malloc(c_dim1 * sizeof(double *));
    for(int abc = 0; abc < c_dim1; abc++){
    	sparse_matrix[abc] = (double*)malloc((*n) * sizeof(double));
	for(int xyz = 1; xyz <= (*n); xyz++) {
		sparse_matrix[abc][xyz-1] = c__[((xyz * c_dim1) + 1) + abc];
	}
    }
	printf("I am here 2: 1329\n");
*/
    
    // Transpose Logic
    std::vector<std::vector<double>> sparse_matrix;
    sparse_matrix.reserve(c_dim1);

    for(int abc = 0; abc < c_dim1; abc++){
	std::vector<double> temp_vector;
	temp_vector.reserve(*n);
	for(int xyz = 1; xyz <= (*n); xyz++) {
		temp_vector.push_back(c__[((xyz * c_dim1) + 1) + abc]);
	}
	sparse_matrix.push_back(temp_vector);	
    }
    ...
    for (int abc = 0; abc < c_dim1; abc++)
    {
    	...
    	{
        	double temp_val = sparse_matrix[abc][xyz];
    ...
    /* Check Pivot Elements
    for (int i = 0; i < c_dim1; i++)
    {
	if (sparse_matrix[i][i] != data[pivot_ptr[i]])
		std::cout<<i<<"\n";
    }
    exit(0);
    */

    // Check CSR and temp matrix
    /*for (int i = 0; i < c_dim1; i++)
    {
	...
	int c_end = row_ptr[i+1];
		...
		if (ctr == 0)
		{
		    std::cout<<"Non-Zero Element not stored\n";
		    break;
		}
...
/* Computing MIN */
	...
	i__2 = i__ + 1;
	i__3 = *mc - i__;
	//printf("10th h12 instance\n");
	//h12t1 = clock();
	h12_copy(&c__1, &i__, &i__2, n, &c__[i__ + c_dim1], lc, &w[iw + i__], &
		c__[j + c_dim1], lc, &c__1, &i__3, pivot_ptr, row_ptr, col_idx, data);
	//h12_(&c__1, &i__, &i__2, n, &c__[i__ + c_dim1], lc, &w[iw + i__], &
	//	c__[j + c_dim1], lc, &c__1, &i__3);
	//h12t2 = clock();
	////printf("time taken by h12 function : %0.5f, line-1179\n", (double)(h12t2 - h12t1) / CLOCKS_PER_SEC);
	i__2 = i__ + 1;
	////printf("11th h12 instance\n");
	//h12t1 = clock();
	h12_copy(&c__2, &i__, &i__2, n, &c__[i__ + c_dim1], lc, &w[iw + i__], &e[
		e_offset], le, &c__1, me, pivot_ptr, row_ptr, col_idx, data);
	//h12_(&c__2, &i__, &i__2, n, &c__[i__ + c_dim1], lc, &w[iw + i__], &e[
	//	e_offset], le, &c__1, me);
	//h12t2 = clock();
	////printf("time taken by h12 function : %0.5f, line-1186\n", (double)(h12t2 - h12t1) / CLOCKS_PER_SEC);
/* L10: */
	i__2 = i__ + 1;
	////printf("12th h12 instance\n");
	//h12t1 = clock();
	h12_copy(&c__2, &i__, &i__2, n, &c__[i__ + c_dim1], lc, &w[iw + i__], &g[
		g_offset], lg, &c__1, mg, pivot_ptr, row_ptr, col_idx, data);
	//h12_(&c__2, &i__, &i__2, n, &c__[i__ + c_dim1], lc, &w[iw + i__], &g[
	//	g_offset], lg, &c__1, mg);
	//h12t2 = clock();
	////printf("time taken by h12 function : %0.5f, line-1194\n", (double)(h12t2 - h12t1) / CLOCKS_PER_SEC);
    }
	//printf("I am here 3: 1358\n");
/*
    for(int abc = 0; abc < c_dim1; abc++){
	for(int xyz = 1; xyz <= (*n); xyz++) {
		c__[((xyz * c_dim1) + 1) + abc] = sparse_matrix[abc][xyz-1];
	}
    }
*/
    ...
    for (int i = 0; i < row_ptr.size() - 1; i++)
    {
		...
		for (int j = c_start; j < c_end; j++)
		{
			double val = data[j];
			int col = col_idx[j];
			//c__[(((col-1) * c_dim1) + 1) + i] = val;
			sparse_matrix[i][col] = val;
	...
	printf("I am here 4: 1364\n");
	h12t2 = clock();
	printf("time taken by h12 function in 10/11/12 instances is : %0.5f, line-1197\n", (double)(h12t2 - h12t1) / CLOCKS_PER_SEC);
	////printf("Inside slsqp: if mode == -1: 18\n");
/*  SOLVE C*X=D AND MODIFY F */
    *mode = 6;
	//////printf("I am here lsei_ 2, where mode = %d\n", *mode);
    i__2 = *mc;
    for (i__ = 1; i__ <= i__2; ++i__) {
	//////printf("1 : d__1 : %.20f, %.20f, %.20f, epmach : %.20f\n", d__1, c__[i__ + i__ * c_dim1], fabs(d__1), epmach);
	if ((d__1 = c__[i__ + i__ * c_dim1], fabs(d__1)) < epmach) {
		//////printf("2 : d__1 : %.20f, %.20f, %.20f, epmach : %.20f\n", d__1, c__[i__ + i__ * c_dim1], fabs(d__1), epmach);
	    goto L75;
	}
	i__1 = i__ - 1;
...
/* L15: */
	...
	////printf("Inside slsqp: if mode == -1: 19\n");
    *mode = 1;
    w[mc1] = 0.0;
...
/* L20: */
	...
	////printf("Inside slsqp: if mode == -1: 20\n");
...
/* L25: */
	...
	////printf("Inside slsqp: if mode == -1: 21\n");
    i__2 = *mg;
    for (i__ = 1; i__ <= i__2; ++i__) {
/* L30: */
	dcopy___(&l, &g[i__ + mc1 * g_dim1], *lg, &w[ig - 1 + i__], *mg);
    }
	////printf("Inside slsqp: if mode == -1: 22\n");
    if (*mg > 0) {
	goto L40;
    }
/*  SOLVE LS WITHOUT INEQUALITY CONSTRAINTS */
    *mode = 7;
    ...
    hfti_(&w[ie], me, me, &l, &w[if__], &k, &c__1, &t, &krank, xnrm, &w[1], &
	    w[l + 1], &jw[1]);
	////printf("Inside slsqp: if mode == -1: 23\n");
...
/* L45: */
	...
	////printf("Inside slsqp: if mode == -1: 24\n");
    lsi_(&w[ie], &w[if__], &w[ig], &h__[1], me, me, mg, mg, &l, &x[mc1], xnrm,
	     &w[mc1], &jw[1], mode);
	////printf("Inside slsqp: if mode == -1: 25\n");
    if (*mc == 0) {
	goto L75;
    }
    t = dnrm2___(mc, &x[1], 1);
...
/* L55: */
	...
	////printf("Inside slsqp: if mode == -1: 26\n");
    i__2 = *mc;
    for (i__ = 1; i__ <= i__2; ++i__) {
/* L60: */
	...
	////printf("Inside slsqp: if mode == -1: 27\n");
    for (i__ = *mc; i__ >= 1; --i__) {
/* L65: */
	i__2 = i__ + 1;
	//printf("13th h12 instance\n");
	h12t1 = clock();
	...
	////printf("Inside slsqp: if mode == -1: 28\n");
    for (i__ = *mc; i__ >= 1; --i__) {
/* Computing MIN */
	i__2 = i__ + 1;
	j = MIN2(i__2,*lc);
...
/* L70: */
	...
	////printf("Inside slsqp: if mode == -1: 29\n");
...
/*  RECOVER MATRIX E AND VECTOR F FROM L AND G */
    ...
    if__ = *n * *n + 1;
    i__1 = n3;
	////printf("Inside slsqp: if mode == -1: 9\n");
    for (i__ = 1; i__ <= i__1; ++i__) {
	i1 = n1 - i__;
	diag = sqrt(l[i2]);
	w[i3] = 0.0;
...
/* L10: */
	...
	////printf("Inside slsqp: if mode == -1: 10\n");
    if (n2 == 1) {
	w[i3] = l[*nl];
	w[i4] = 0.0;
    ...
    ic = if__ + *n;
    id = ic + *meq * *n;
	////printf("Inside slsqp: if mode == -1: 11\n");
    if (*meq > 0) {
...
/*  RECOVER VECTOR D FROM UPPER PART OF B */
	...
	////printf("Inside slsqp: if mode == -1: 12\n");
    ig = id + *meq;
    if (mineq > 0) {
...
/* L30: */
	...
	////printf("Inside slsqp: if mode == -1: 13\n");
...
/* L40: */
	...
	////printf("Inside slsqp: if mode == -1: 14\n");
    i__1 = m1 + 1;
...
/* L50: */
	...
	////printf("Inside slsqp: if mode == -1: 15\n");
    i__1 = m1 + 1;
    /* SGJ, 2010: skip constraints for infinite bounds */
    for (i__ = 1; i__ <= *n; ++i__)
...
L260:
	////printf("Inside slsqp: if mode == -1: 1\n");
    i__1 = *n;
    for (i__ = 1; i__ <= i__1; ++i__) {
...
/* L290: */
	...
	////printf("Inside slsqp: if mode == -1: 2\n");
/*   D*L'*S */
    k = 1;
    i__1 = *n;
    for (i__ = 1; i__ <= i__1; ++i__) {
...
/* L300: */
	...
	////printf("Inside slsqp: if mode == -1: 3\n");
/*   L*D*L'*S */
    for (i__ = *n; i__ >= 1; --i__) {
	h1 = 0.0;
	k = i__;
...
/* L320: */
	...
	////printf("Inside slsqp: if mode == -1: 4\n");
	...
	////printf("Inside slsqp: if mode == -1: 5\n");
...
#define U(n) ((unsigned) (n))
	...
	if(var1 == 1) {
		...
		//printf("time taken inside slsqp when mode is -1/-2 : %0.5f\n", time_taken);
	}
