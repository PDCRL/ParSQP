...
L40:
	...
	if(*mode == 1) {
		...
		//for (i__ = *l1; i__ <= i__2; ++i__) {
		    //sm += c__[i3] * u[i__ * u_dim1 + 1];
		    //sm += c__[i3] * temp_matrix[*lpivot - 1][i__ - 1];
		    //sm += eg_temp_matrix[j - 1][i__ - 1]  * temp_matrix[*lpivot - 1][i__ - 1];
		    //i3 += *ice;
		//}
		...
		//for (i__ = *l1; i__ <= i__2; ++i__) {
		    //c__[i4] += sm * u[i__ * u_dim1 + 1];
		    //c__[i4] += sm * temp_matrix[*lpivot - 1][i__ - 1];
		    //eg_temp_matrix[j - 1][i__ - 1] += sm * temp_matrix[*lpivot - 1][i__ - 1];
		    //i4 += *ice;
		//}
                for (i__ = c_start+1; i__ < c_end; i__++){
...
static void lsi_(double *e, double *f, double *g, 
	...
	jw, int *mode, double **g_temp_matrix)
{
    /* Initialized data */
...
/*
	    g[i__ + j * g_dim1] = (g[i__ + j * g_dim1] - ddot_sl__(&i__3, &g[
		    i__ + g_dim1], *lg, &e[j * e_dim1 + 1], 1)) / e[j + j *
		     e_dim1];
*/
	    g_temp_matrix[i__-1][e_dim1-*n + j] =  (g_temp_matrix[i__-1][e_dim1-*n + j] - ddot_sl__using_tempMat(&i__3, g_temp_matrix, i__ - 1, &e[j * e_dim1 + 1], 1, e_dim1-*n)) / e[j + j * e_dim1];

	}
/* L30: */
	//h__[i__] -= ddot_sl__(n, &g[i__ + g_dim1], *lg, &f[1], 1);
	h__[i__] -= ddot_sl__using_tempMat(n, g_temp_matrix, i__-1, &f[1], 1, e_dim1-*n);
    }

    for(int xyz = 0; xyz < *n; xyz++) {
    	for(int abc = 0; abc < *lg; abc++){
		g[g_offset + ((xyz * *lg) + abc)] = g_temp_matrix[abc][e_dim1 - *n + xyz];
	}
    }

    //h12t2 = clock();
    double lsi_end = omp_get_wtime();
    //printf("Time taken by h12 function in lsi : %0.5f\n", (double)(h12t2 - h12t1) / CLOCKS_PER_SEC);
    printf("Time taken by h12 function in lsi : %0.5f\n", lsi_end-lsi_start);
...
/*  SOLVE C*X=D AND MODIFY F */
    *mode = 6;
	...
	//if ((d__1 = temp_matrix[i__ - 1][i__ - 1], fabs(d__1)) < epmach) {
	    goto L75;
	}
	i__1 = i__ - 1;
	x[i__] = (d__[i__] - ddot_sl__(&i__1, &c__[i__ + c_dim1], *lc, &x[1], 1)) 
	     / c__[i__ + i__ * c_dim1];
	//x[i__] = (d__[i__] - ddot_sl__using_tempMat(&i__1, temp_matrix, *lc, &x[1], 1, false)) 
	//     / temp_matrix[i__ - 1][i__ - 1];
/* L15: */
    }
    *mode = 1;
    w[mc1] = 0.0;
...
/* L45: */
    ...
    t2 = clock();
    printf("Time taken inside lsei after : %0.5f\n", (double)(t2 - t1) / CLOCKS_PER_SEC); 
/*
    for(int xyz = 0; xyz < *n; xyz++) {
    	for(int abc = 0; abc < *lg; abc++){
		std::cout<<g_temp_matrix[abc][xyz]<<" ";
	}
    	std::cout<<'\n';
    }
    ...
    for(int xyz = 0; xyz < 1250; xyz++) {
    	for(int abc = 0; abc < *lg; abc++){
		std::cout<<g[g_offset + ((xyz * *lg) + abc)]<<" ";
	}
	std::cout<<'\n';
    }
    exit(0);
...
/* L65: */
	i__2 = i__ + 1;
	//h12_copy(&c__2, &i__, &i__2, n, &c__[i__ + c_dim1], lc, &w[iw + i__], &x[
	//	1], &c__1, &c__1, &c__1, temp_matrix);
    ...
    //h12t4 = clock();
    //printf("time taken by h12 function is : %0.5f, in line-11784\n", (double)(h12t4 - h12t3) / CLOCKS_PER_SEC);
...
/* Computing MIN */
	...
	//w[i__] = (d__[i__] - ddot_sl__using_tempMat(&i__2, temp_matrix, *lc, &
	//	w[j], 1, true)) / temp_matrix[i__ - 1][i__ - 1];
/* L70: */
    }
/*  END OF SUBROUTINE LSEI */
L75:  
    //printf("Before lsei return\n");
    return;
} /* lsei_ */
...
{
    slsqpb_ctr++;
    //std::cout<<"slsqpb_ called for "<< slsqpb_ctr<<" times\n";
